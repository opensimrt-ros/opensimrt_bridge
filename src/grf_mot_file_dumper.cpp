...
class Gfrm: public Ros::CommonNode
{
	...
	Gfrm()
	{
		...
		output.labels = conv_labels(labels_); // am I messing the order here?
	...
	void onInit()
	{
		...
		output.set(output.labels); // important is that this is done after I read the tableseries so that I have the columnnames set on the labels variable 	
		output.labels = output.desired_label_order;
